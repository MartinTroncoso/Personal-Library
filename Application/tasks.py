...
def libro_del_dia():
    ...
    libros_validos = [item for item in data["items"]
    ...
    if not libros_validos:
        return "❌ No se encontraron libros con páginas visibles"
    ...
    return f"✅ Libro guardado: {volumeInfo.get('title')}, descripcion: {volumeInfo.get('description')}, subtitulo: {volumeInfo.get('subtitle')}"